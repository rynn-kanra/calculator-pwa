	...
	self.onmessage = function (event) {
		...
		Module['onRuntimeInitialized'] = function () {
			self.onmessage = (event) => {
				...
				try {
					let data = onMessage(event);
					post({ id: id, data: data });
				}
				catch (error) {
					post({ id: id, type: "error", data: error });
				}
			};
			self.postMessage({});
		};
...
startup(function (event) {
	switch (event.data.command) {
		case 'initialize':
			return initialize(event.data.data, event.data.callbackId);
		case 'load':
			return load(event.data.data, event.data.callbackId);
		case 'lazyLoad':
			return lazyLoad(event.data.data, event.data.callbackId);
		case 'addWords':
			return addWords(event.data.data, event.data.callbackId);
		case 'addGrammar':
			return addGrammar(event.data.data, event.data.callbackId);
		case 'lookupWord':
			return lookupWord(event.data.data, event.data.callbackId);
		case 'lookupWords':
			return lookupWords(event.data.data, event.data.callbackId);
		case 'addKeyword':
			return addKeyword(event.data.data, event.data.callbackId);
		case 'start':
			return start(event.data.data);
		case 'stop':
			return stop();
		case 'process':
			return process(event.data.data);
	}
});
...
function initialize(data, clbId) {
	...
	if (recognizer) {
		...
		if (recognizer === undefined) {
			throw new Error(Module.ReturnType.RUNTIME_ERROR);
		}
		else {
			return clbId;
		}
...
function addWords(data, clbId) {
	if (recognizer) {
		...
		throw new Error("js-no-recognizer");
	}
}
